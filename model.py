...
class Player(movableObject):
	...
	def think(self):
		...
		self.vx  = w1[0]*w1dd*1000000;
		self.vy  = w1[1]*w1dd*1000000;
		...
		if self.energy > 7*self.energy_step and d < 13:
			k2 = gauss(0, 15, d)/gauss(0, 15, 0);
			c = self.field.goals[(self.team+1)%2].get_center();
			self.field.ball.kick(k2*(c[0]-self.x)*self.energy/self.energy_max, k2*(c[1]-self.y)*self.energy/self.energy_max);
